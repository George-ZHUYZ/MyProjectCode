...
function updateData($tableName, $idValue, $fieldName, $fieldType, $fieldValue) {
    ...
    $updateOptionNew = str_replace("AND", ",", $updateOption);
    $sql = "UPDATE $tableName SET $updateOptionNew WHERE ID = '$idValue'";
    if (!mysql_query($sql, $con)) {
...
function getUniqeValue($tableName, $field, $condition) {
    $con = selectDatabase($GLOBALS['server_databasename']);
    $sql = "SELECT DISTINCT $field FROM $tableName WHERE $condition ;";
    $output = array();
    $result = mysql_query($sql, $con);
...
function getpage($pageID) {
    ...
    $condition = getConditions(array("id", "belong_site"), array("int", "string"), array($pageID, $sitebelong));
    //print $condition;
...
function updateComment($ID, $Comment, $belong_site, $date) {
    $cond = getConditions(array("belong_site", "IssueID", "Period"), array("string", "string", "String"), array($belong_site, $ID, $date));
...
function updateRating($ID, $total, $belong_site, $date) {
    $cond = getConditions(array("belong_site", "ProductID", "Period"), array("string", "string", "String"), array($belong_site, $ID, $date));
