...
function formatePrice($inputPrice) {
	$formatedPrice = number_format(($inputPrice / 100), 2, ".", ",");
...
function unicode($input) {
	if (empty($input)) {
		...
		$outputs = mb_convert_encoding($input, 'HTML-ENTITIES', 'utf-8');
		$outputs=  explode(";", $outputs);
		$words=array();
		for ($i = 0; $i < count($outputs)-1; $i++) {
			$output = (int) substr($outputs[$i], 2, 5);
			$output = dechex($output);
			$output = '&#x' . $output . ';';
			$words[$i]=$output;
		}
